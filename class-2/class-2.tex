    ...
    \Large
        \begin{itemize}
            ...
            \large
                ...
                \bigskip
                \item La partie de l'arbre sur laquelle la règle s'applique est en général indiquée dans le \texttt{@select} de l'instruction XSL (cf. infra). % même si c'est la même que celle contenue dans le @match
            \end{itemize}
        \end{itemize}
    \end{frame}
...
<xsl:template match="/TEI/text/body">
    <xsl:copy>...</xsl:copy>
     ...
     \begin{itemize}
         ...
         \item C'est un élément vide (pas de règles internes).
