    ...
    <xsl:element name="body">
     ...
     \begin{frame}{XSLT/ Principales règles (1/4) : \texttt{<xsl:apply-template/>}}
         ...
         \begin{itemize}
            ...
            \bigskip
            \item Exemple : comment transformer tous les \texttt{<hi rend="bold"/>} en \texttt{<bold>} ?
            \item Écrire la règle une seule fois et l'appliquer partout avec des \texttt{<xsl:apply-template/>}.
         \end{itemize}
     \end{frame}
...
% expliquer la différence entre le noeud courant et le noeud de contexte ?
     ...
     \Large
     \begin{itemize}
         \item \textbf{Copie de l'ensemble du n\oe ud courant et de ses n\oe uds enfants (balises, attributs, textes).}
...
<xsl:template match="/TEI/text/body">
    <xsl:copy-of select="."/>
     ...
     \Large
     \begin{itemize}
         \item \textbf{Renvoie la valeur textuelle du n\oe ud courant.}
         \item Le n\oe ud courant est toujours contenu dans le \texttt{@select}.
     \end{itemize}
     \begin{minted}{xml}
<xsl:template match="/TEI/text/body">
    ...
    <xsl:copy>
     ...
     \begin{frame}{XSLT/ Une autre règle : \texttt{<xsl:number/> (1/2)}}
         ...
         \begin{itemize}
             ...
             \item \texttt{@count} $\rightarrow$ défini les éléments de l'input qui seront numérotés dans l'output ;
...
<xsl:template match="//body/p">
    <xsl:copy>
     ...
     \begin{frame}{XSLT/ Exercice}
         \Large
         \begin{itemize}
             \item Écrivez une feuille de style XSL avec le fichier de St Julien l'Hospitalier,
