...
\begin{document}
  ...
  \section{XSLT. Règles basiques}
    ...
    \end{frame}
     ...
     \begin{frame}{Contenu d'un \texttt{<xsl:template>} : fichiers d'exemple}
         \Large
         \begin{itemize}
            \item Ouvrir Oxygen.
             \item Ouvrir \texttt{/class-2/class2\_teiAll\_oxygen.xml}.
             \item Ouvrir \texttt{/class-2/class2\_teiAll\_xsl.xsl}.
         \end{itemize}
     \end{frame}
...
<xsl:template match="/TEI/text/body">
    <xsl:element name="body">
     ...
     \begin{frame}[fragile]{XSLT/ Principales règles (1/4) : \texttt{<xsl:copy>}}
     \Large
     \begin{itemize}
...
<xsl:template match="/TEI/text/body">
    <xsl:copy>...</xsl:copy>
     ...
     \begin{frame}[fragile]{XSLT/ Principales règles (2/4) : \texttt{<xsl:copy-of>}}
     \Large
     \begin{itemize}
...
<xsl:template match="/TEI/text/body">
    <xsl:copy-of select="."/>
     ...
     \begin{frame}[fragile]{XSLT/ Principales règles (3/4) : \texttt{<xsl:value-of>}}
     \Large
     \begin{itemize}
...
<xsl:template match="//body/p">
    ...
    \begin{frame}{XSLT/ Principales règles (4/4) : \texttt{<xsl:apply-templates/>}}
         \Large
         \begin{itemize}
             \item \texttt{<xsl:apply-templates/>} est une \textbf{instruction récursive} : le processeur va \textbf{examiner les n\oe uds enfants} dans l’ordre et \textbf{appliquer les règles} qui leur sont associées.
    ...
    \begin{frame}[fragile]{XSLT/ La règle \texttt{<xsl:apply-templates/>} (1/4)}
        \Large
        \begin{itemize}
...
<xsl:template match="/">
    ...
    \begin{frame}[fragile]{XSLT/ La règle \texttt{<xsl:apply-templates/>} : \texttt{@select} (2/4)}
        \Large
        \begin{itemize}
...
<xsl:template match="/">
    ...
    \begin{frame}{XSLT/ La règle \texttt{<xsl:apply-templates/>} : \texttt{@mode} (3/4)}
        \Large
        \begin{itemize}
    ...
    \begin{frame}[fragile]{XSLT/ La règle \texttt{<xsl:apply-templates/>} : \texttt{@mode} (4/4)}
        \Large
        \begin{itemize}
    ...
    \end{frame}
     ...
     \begin{frame}{XSLT/ Exercice}
         ...
         \begin{itemize}
             ...
             \item Bonus pour les plus aguérris : ajouter le début du §3 (lien dans le \texttt{<teiHeader/>}).
         \end{itemize}
     \end{frame}
